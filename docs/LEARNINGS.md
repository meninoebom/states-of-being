...
**Solution (chopper):** Non-vocal stems now cut on downbeats **only** (no silence snap), then trim/zero-pad each loop so its length is an exact multiple of the *nominal* bar (`60/bpm * time_signature`). Exactness is defined against the nominal grid, not the raw downbeat span, because real songs drift from a constant tempo and the frontend Transport runs at the nominal tempo. `chop_stem` now takes `bpm`/`time_signature`. `duration_sec` is stored at sample-fine precision (6 dp) and is the exact nominal-bar length.
